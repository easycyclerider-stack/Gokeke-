...
export default function DriverProfile(){
 ...
 <div style={{padding:14,border:'1px solid #ddd',borderRadius:12,margin:'12px 0'}}><h3>Verification checklist</h3><p>Driver approval: {driver?.status==='approved'?'✓ Approved':'○ '+(driver?.status||'Pending')}</p><p>Licence number: {driver?.license_number?'✓ Submitted':'○ Required'}</p><p>Licence document: {driver?.license_document_url?'✓ Submitted':'○ Required'}</p><p>ID document: {driver?.id_document_url?'✓ Submitted':'○ Required'}</p><p>Vehicle: {vehicle?.is_active?'✓ Approved & active':'○ Pending admin approval'}</p><p>Vehicle photo: {vehicle?.photo_url?'✓ Submitted':'○ Required'}</p><p style={{fontWeight:700}}>Dispatch eligibility: {driver?.status==='approved'&&driver?.license_number&&driver?.license_document_url&&driver?.id_document_url&&vehicle?.is_active?'✓ Fully verified — eligible to go online':'○ Not yet eligible'}</p></div><h2>Verification status</h2><p><b>Driver:</b> {driver?.status||'pending'} {driver?.verified_at?'✓ verified':''}</p><p><b>Application:</b> {app?.reviewed_at?'Reviewed':'Pending review'}</p>{app?.notes&&<p><b>Admin note:</b> {app.notes}</p>}
