...
export async function updateSession(request: NextRequest) {
  ...
  if(data?.claims){const {data:p}=await supabase.from('profiles').select('role').eq('id',data.claims.sub).maybeSingle();const role=p?.role;if(pathname.startsWith('/admin')&&role!=='admin')return NextResponse.redirect(new URL('/auth',request.url));if(pathname.startsWith('/driver')&&role!=='driver')return NextResponse.redirect(new URL('/auth',request.url));if((pathname.startsWith('/passenger')||pathname.startsWith('/ride'))&&role!=='passenger')return NextResponse.redirect(new URL('/auth',request.url));}
