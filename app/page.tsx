...
import { Bike, CarFront, ShieldCheck, Radio, Wallet, MapPin, UserRound, LayoutDashboard } from 'lucide-react';

export default function Home() {
  const [vehicleType, setVehicleType] = useState<'okada' | 'keke'>('okada');
  ...
  return (
    <>
      ...
      <section className="hero">
        <div>
          <div className="eyebrow">Made for everyday Nigeria</div>
        ...
        <div className="panel">
          <div className="map"><div className="road" /><div className="pin"><i /></div></div>
          <div className="field"><MapPin size={15} /> Pickup location</div>
          <div className="field"><MapPin size={15} /> Destination</div>
          <div className="types">
            <button className={'type ' + (vehicleType === 'okada' ? 'active' : '')} onClick={() => setVehicleType('okada')}><Bike /> <b>Okada</b><br /><small>Fast & nimble</small></button>
            <button className={'type ' + (vehicleType === 'keke' ? 'active' : '')} onClick={() => setVehicleType('keke')}><CarFront /> <b>Keke</b><br /><small>More space</small></button>
          </div>
          <div className="estimate"><span>Estimated fare</span><span>₦{vehicleType === 'okada' ? '500+' : '700+'}</span></div>
        </div>
      </section>

      <section className="section">
        <div className="eyebrow" style={{ color: '#d8ff3e' }}>The platform</div>
      ...
      <section className="hero">
        ...
        <div className="panel">
          ...
          <p style={{ color: '#666', lineHeight: 1.6 }}>GoKeke is designed around identity, licence and vehicle-document verification.</p>
          <div style={{ display: 'flex', gap: 18, marginTop: 20 }}><Radio /><Wallet /></div>
        </div>
      </section>

      <footer className="footer">© 2026 GoKeke · Okada · Keke · Built for Nigeria</footer>
    </>
  );
}
