...
export default function Passenger() {
  ...
  if (!session) return <main style={{maxWidth:520,margin:'70px auto',padding:24}}><div className="panel"><h1>GoKeke Passenger</h1><p>{mode==='login'?'Sign in to book a ride':'Create your passenger account'}</p><form onSubmit={async(e:FormEvent)=>{e.preventDefault();setMsg('');if(mode==='login'){const{data,error}=await supabase.auth.signInWithPassword({email,password:pw});if(error)setMsg(error.message);else{setSession(data.session);router.push('/passenger')}}else{const{data,error}=await supabase.auth.signUp({email,password:pw,options:{data:{full_name:name,role:'passenger'}}});if(error)setMsg(error.message);else if(data.user){await supabase.from('profiles').upsert({id:data.user.id,full_name:name,role:'passenger'});setMsg('Account created. Check email confirmation if enabled.')}}}}>{mode==='signup'&&<input required placeholder="Full name" value={name} onChange={e=>setName(e.target.value)}/>}<input required type="email" placeholder="Email" value={email} onChange={e=>setEmail(e.target.value)}/><input required minLength={8} type="password" placeholder="Password" value={pw} onChange={e=>setPw(e.target.value)}/><button className="btn lime">{mode==='login'?'Sign in':'Create account'}</button></form><button className="btn" onClick={()=>setMode(mode==='login'?'signup':'login')}>{mode==='login'?'Create account':'I already have an account'}</button>{msg&&<p>{msg}</p>}</div></main>;
